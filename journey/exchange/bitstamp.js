...
async function getCurrentMarket() { try {
  ...
  var marketCache
  if (mock) {
    marketCache = await bitstampdata.readMarket(symbols.XBTUSD,60,startTime,endTime)
  }
  else {
    marketCache = await bitstampdata.getMarket(symbols.XBTUSD,60,startTime,endTime)
  }
